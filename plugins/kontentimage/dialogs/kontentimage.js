...
CKEDITOR.dialog.add('kontentimage', function (editor) {
	...
	var regexGetSizeOrEmpty = /(^\s*(\d+)(px)?\s*$)|^$/i,
		...
		helpers = CKEDITOR.plugins.kontentimage,
		...
		// Functions inherited from kontentimage plugin.
	...
	function onChangeSrc() {
		...
		if (value !== widget.data.src) {
			...
			preLoader(value, function (image, width, height) {
				...
				widthField.setValue(editor.config.kontentimage_prefillDimensions === false ? 0 : width);

				// Fill height field with the height of the new image.
				heightField.setValue(editor.config.kontentimage_prefillDimensions === false ? 0 : height);
	...
	/************************************************** CUSTOM CODE *******************************************************************/
	if (config.customJsImageBrowser) {
		srcBoxChildren.push(
			{
				type: 'button',
				id: 'jsbrowse',
				...
				hidden: false,
				onClick: function () {
					config.customJsImageMethod().then(assetUrl => {
						if (assetUrl) this.getDialog().getContentElement("info", "src").setValue(assetUrl)
					});
				}
			});
	}
		...
		contents: [
			{
				...
				elements: [
					...
					},
					{
						...
						validate: editor.config.kontentimage_altRequired === true ? CKEDITOR.dialog.validate.notEmpty(lang.altMissing) : null
					},
					{
						type: 'hbox',
						widths: ['25%', '25%', '50%'],
