...
( function() {
	...
	CKEDITOR.plugins.add( 'kontentimage', {
		...
		init: function( editor ) {
			...
			if ( editor.plugins.detectConflict( 'kontentimage', [ 'easyimage' ] ) ) {
				return;
			}

			// Adapts configuration from original image plugin. Should be removed
			// when we'll rename kontentimage to image.
			var config = editor.config,
				lang = editor.lang.kontentimage,
				image = widgetDef( editor );

			// Since filebrowser plugin discovers config properties by dialog (plugin?)
			// names (sic!), this hack will be necessary as long as kontentimage is not named
			// Image. And since kontentimage will never be Image, for sure some filebrowser logic
			// got to be refined.
			config.filebrowserkontentimageBrowseUrl = config.filebrowserImageBrowseUrl;
			config.filebrowserkontentimageUploadUrl = config.filebrowserImageUploadUrl;
			...
			CKEDITOR.dialog.add( 'kontentimage', this.path + 'dialogs/kontentimage.js' );
		},
	...
	// 		└──────┴───────────────────────────────┴─────────────────────────────┘
	//
	// Non-captioned widget (config.kontentimage_alignClasses defined)
	// 		┌──────┬───────────────────────────────┬─────────────────────────────┐
	// 		│Align │Internal form                  │Data                         │
	// 		├──────┼───────────────────────────────┼─────────────────────────────┤
	// 		│none  │<wrapper>                      │<img />                      │
	...
	// 		└──────┴────────────────────────────────────────┴────────────────────────────────────────┘
	//
	// Captioned widget (config.kontentimage_alignClasses defined)
	// 		┌──────┬────────────────────────────────────────┬────────────────────────────────────────┐
	// 		│Align │Internal form                           │Data                                    │
	// 		├──────┼────────────────────────────────────────┼────────────────────────────────────────┤
	// 		│none  │<wrapper>                               │<figure />                              │
	...
	function widgetDef( editor ) {
		var alignClasses = editor.config.kontentimage_alignClasses,
			captionedClass = editor.config.kontentimage_captionedClass;
		...
		return {
			...
			init: function() {
				var helpers = CKEDITOR.plugins.kontentimage,
				...
				if ( editor.filter.checkFeature( this.features.dimension ) && editor.config.kontentimage_disableResizer !== true ) {
					setupResizer( this );
				}
			...
			// Overrides default method to handle internal mutability of kontentimage.
			...
			getClasses: ( function() {
				...
				return function() {
					...
					// Neither config.kontentimage_captionedClass nor config.kontentimage_alignClasses
	...
	/**
	 * A set of Enhanced Image (kontentimage) plugin helpers.
	 *
	 * @class
	 * @singleton
	 */
	CKEDITOR.plugins.kontentimage = {
		stateShifter: function( editor ) {
			// Tag name used for centering non-captioned widgets.
			var doc = editor.document,
				alignClasses = editor.config.kontentimage_alignClasses,
				captionedClass = editor.config.kontentimage_captionedClass,
				...
				hasCaption:	function( shift, oldValue, newValue ) {
					...
					if ( newValue ) {
						...
						var figure = CKEDITOR.dom.element.createFromHtml( templateBlock.output( {
							...
							captionPlaceholder: editor.lang.kontentimage.captionPlaceholder
						} ), doc );
				...
				link: function( shift, oldValue, newValue ) {
					if ( shift.changed.link ) {
						...
						if ( !newValue )
							...
							var attributes = CKEDITOR.plugins.kontentimage.getLinkAttributesGetter()( editor, newValue );
	...
	// <figure class="{config.kontentimage_captionedClass}"> elements.
	//
	...
	function downcastWidgetElement( editor ) {
		...
		return function( el ) {
			...
			// inline styles or classes (kontentimage_alignClasses).
	...
	function centerWrapperChecker( editor ) {
		var captionedClass = editor.config.kontentimage_captionedClass,
			alignClasses = editor.config.kontentimage_alignClasses,
		...
		return function( el ) {
			...
			// Centering wrapper got to be... centering. If kontentimage_alignClasses are defined,
	...
	function setupResizer( widget ) {
		...
		resizer.setAttribute( 'title', editor.lang.kontentimage.resizer );
		...
		resizer.on( 'mousedown', function( evt ) {
			...
			function getMaxSize() {
				var maxSize = editor.config.kontentimage_maxSize,
					natural;
				...
				natural = CKEDITOR.plugins.kontentimage.getNatural( image );
			...
			if ( dialog.name == 'link' ) {
				...
				def.onShow = function() {
					...
					if ( widget && ( widget.inline ? !widget.wrapper.getAscendant( 'a' ) : 1 ) ) {
						...
						// Hide the display text in case of linking kontentimage widget.
						displayTextField.hide();
					} else {
						// Make sure that display text is visible, as it might be hidden by kontentimage integration
						// before.
	...
	// on configurations like config#kontentimage_alignClasses or
	// config#kontentimage_captionedClass.
	//
	// @param {CKEDITOR.editor}
	// @returns {Object}
	function getWidgetAllowedContent( editor ) {
		var alignClasses = editor.config.kontentimage_alignClasses,
			rules = {
				...
				figure: {
					classes: '!' + editor.config.kontentimage_captionedClass
				},
				figcaption: true
			};
 ...
 *
 * Read more in the {@glink features/kontentimage documentation} and see the
 * {@glink examples/kontentimage example}.
 *
 *		// Changes the class to "captionedImage".
 *		config.kontentimage_captionedClass = 'captionedImage';
 *
 * @cfg {String} [kontentimage_captionedClass='image']
 * @member CKEDITOR.config
 */
CKEDITOR.config.kontentimage_captionedClass = 'image';
 ...
 * {@glink examples/kontentimage example}.
 *
 *		config.kontentimage_prefillDimensions = false;
 *
 * @since 4.5.0
 * @cfg {Boolean} [kontentimage_prefillDimensions=true]
 * @member CKEDITOR.config
 */
 ...
 * {@glink examples/kontentimage example}.
 *
 *		config.kontentimage_disableResizer = true;
 *
 * @since 4.5.0
 * @cfg {Boolean} [kontentimage_disableResizer=false]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.kontentimage_alignClasses = [ 'align-left', 'align-center', 'align-right' ];
 *
 ...
 *
 * @since 4.4.0
 * @cfg {String[]} [kontentimage_alignClasses=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.kontentimage_altRequired = true;
 *
 * Read more in the {@glink features/kontentimage documentation} and see the
 * {@glink examples/kontentimage example}.
 *
 * @since 4.6.0
 * @cfg {Boolean} [kontentimage_altRequired=false]
 * @member CKEDITOR.config
 */
 ...
 *
 * @since 4.12.0
 * @cfg {Object.<String, Number/String>} [kontentimage_maxSize]
 * @member CKEDITOR.config
 */
