...
(function () {
	CKEDITOR.plugins.add('kontentlink', {
		...
		init: function (editor) {
			...
			editor.on('doubleclick', function (evt) {
				...
				if (element && !element.isReadOnly()) {
					...
					} else if (CKEDITOR.plugins.kontentlink.tryRestoreFakeAnchor(editor, element)) {
			...
			if (editor.contextMenu) {
				editor.contextMenu.addListener(function (element) {
					...
					var anchor = CKEDITOR.plugins.kontentlink.tryRestoreFakeAnchor(editor, element);

					if (!anchor && !(anchor = CKEDITOR.plugins.kontentlink.getSelectedLink(editor))) {
						return null;
					}
		...
		afterInit: function (editor) {
			...
			if (pathFilters) {
				pathFilters.push(function (element, name) {
					if (name == 'a') {
						if (CKEDITOR.plugins.kontentlink.tryRestoreFakeAnchor(editor, element) || (element.getAttribute('name') && (!element.getAttribute('href') || !element.getChildCount()))) {
							return 'anchor';
						}
					}
				});
	...
	function protectEmailLinkAsFunction(editor, email) {
		var plugin = editor.plugins.kontentlink,
	 ...
	 * @singleton
	 */
	CKEDITOR.plugins.kontentlink = {
		/**
		 * Get the surrounding link element of the current selection.
		 *
		 *		CKEDITOR.plugins.kontentlink.getSelectedLink( editor );
		 *
		...
		parseLinkAttributes: function (editor, element) {
			var href = (element && (element.data('cke-saved-href') || element.getAttribute('href'))) || '',
				compiledProtectionFunction = editor.plugins.kontentlink.compiledProtectionFunction,
	...
	CKEDITOR.removeAnchorCommand.prototype = {
		exec: function (editor) {
			...
			if (sel && (anchor = sel.getSelectedElement()) && (!anchor.getChildCount() ? CKEDITOR.plugins.kontentlink.tryRestoreFakeAnchor(editor, anchor) : anchor.is('a'))) {
				anchor.remove(1);
			} else {
				if ((anchor = CKEDITOR.plugins.kontentlink.getSelectedLink(editor))) {
