...
(function () {
	CKEDITOR.dialog.add('link', function (editor) {
		var plugin = CKEDITOR.plugins.kontentlink,
			initialLinkText;
					...
					children: [{
						...
						children: [{
							...
							commit: function (data) {
								...
								let urlParts = this.getValue().split("://");

								if (urlParts.length > 1) {
									data.url.protocol = urlParts[0] + "://";
									data.url.url = urlParts[1];
								}
								else {
									data.url.url = urlParts[0];
								}
						...
						},
						{
							...
							hidden: !editor.config.customJsLinkBrowser,
							onClick: function () {
								editor.config.customJsLinkMethod().then(contentUrl => {
									if (contentUrl) this.getDialog().getContentElement("info", "url").setValue(contentUrl)
								});
							}
							/****************************** CUSTOM CODE ******************************************/
						}]
